...
"""
 ...
 This implementation comes from https://github.com/OpenNMT/OpenNMT-py
"""
...
class Beam(object):
    def __init__(self, size, n_best=1, cuda=False):
        ...
        self.nextYs = [self.tt.LongTensor(size) .fill_(self.PAD_token)]
        self.nextYs[0][0] = self.SOS_token

        # Has EOS topped the beam yet.
        self._eos = self.EOS_token
        self.eosTop = False
    ...
    def advance(self, wordLk):
        ...
        * `wordLk`- probs of advancing from the last step (K x words)
        ...
        for i in range(self.nextYs[-1].size(0)):
            if ((self.nextYs[-1][i] == self.EOS_token).data > 0).all():
                s = self.scores[i]
                self.finished.append((s, len(self.nextYs) - 1, i))

        # End condition is when top-of-beam is EOS and no global score.
        if ((self.nextYs[-1][0] == self.EOS_token).data > 0).all():
            # self.allScores.append(self.scores)
            self.eosTop = True
